...
class LSTM(L.LightningModule):
    ...
    def lstm_unit(self, input_value, long_memory, short_memory):
        ...
        update_short_memory = torch.tanh(updated_long_memory) * output_percent

        return ([updated_long_memory, update_short_memory])
...
print(model(torch.tensor([1., 0.25, 0.5, 0.])).detach())

model = LSTM()
inputs = torch.tensor([[0.5, 0.5, 0.25, 1.], [1., 0.5, 0.25, 1.]])
labels = torch.tensor([0., 1.])

dataset = TensorDataset(inputs, labels)
dataloader = DataLoader(dataset)

trainer = L.Trainer(max_epochs=300000)
